...
impl MongoDataRepository {
    pub fn new(client: Client, database: String) -> Self {
        debug!(
            "Initializing MongoDB data repository for database: {}",
            database
        );
...
impl DataRepository for MongoDataRepository {
    async fn insert_documents(
        ...
        debug!(
            "Converting {} JSON documents to BSON and adding log_id: {}",
            documents.len(),
            log_id
        );
        let docs: Vec<Document> = documents
            ...
            .map(|(i, doc)| {
                ...
                mongodb::bson::to_document(&doc_with_log_id).map_err(|e| {
                    error!("Failed to convert document {} to BSON: {}", i, e);
                    debug!(
                        "Problematic document: {}",
                        serde_json::to_string_pretty(&doc_with_log_id)
